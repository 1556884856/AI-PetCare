<!--
  我的优惠券页面
  按 Tab 筛选：未使用/已使用/已过期
-->
<template>
  <div class="page"><h1 class="page-title">🎫 我的优惠券</h1>
    <el-tabs v-model="activeTab" @tab-change="load">
      <el-tab-pane label="未使用" name="unused" /><el-tab-pane label="已使用" name="used" /><el-tab-pane label="已过期" name="expired" />
    </el-tabs>
...
const list = ref<any[]>([])
const load = async () => { try { const res: any = await getMyCoupons(activeTab.value); list.value = res.data || [] } catch {} }
load()
...
.coupon-card { display: flex; background: linear-gradient(135deg, #fff8e6, #fff); border: 2px solid #ffd666; border-radius: 12px; overflow: hidden; }
.coupon-card.used, .coupon-card.expired { opacity: 0.6; filter: grayscale(0.5); }
.coupon-left { width: 160px; display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--secondary); color: #fff; padding: 24px 12px; }
.coupon-card.used .coupon-left, .coupon-card.expired .coupon-left { background: #999; }
