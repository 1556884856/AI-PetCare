<!--
  支付记录页面
  列表展示用户的支付历史，含原价、实付、优惠、状态
-->
<template>
  <div class="page"><h1 class="page-title">💳 支付记录</h1>
    <div v-if="list.length === 0" class="empty">暂无支付记录</div>
    <div class="pay-list">
      <div v-for="p in list" :key="p.id" class="pay-item">
        <div class="pay-left"><div class="pay-service">{{ p.serviceName }}</div><div class="pay-pet">🐾 {{ p.petName }}</div><div class="pay-time">{{ formatTime(p.createdAt) }}</div></div>
        <div class="pay-right">
          <div class="pay-amount"><span class="final">¥{{ p.finalAmount.toFixed(2) }}</span><span v-if="p.discountAmount > 0" class="discount">省 ¥{{ p.discountAmount.toFixed(2) }}</span></div>
...
const list = ref<any[]>([])
onMounted(async () => { try { const res: any = await getMyPayments(); list.value = res.data || [] } catch {} })
const statusLabel = (s: number) => { const map: Record<number, string> = { 0: '待支付', 1: '已支付', 2: '已退款' }; return map[s] || '未知' }
const statusType = (s: number) => { const map: Record<number, string> = { 0: 'warning', 1: 'success', 2: 'info' }; return map[s] || 'info' }
...
.pay-item { display: flex; justify-content: space-between; align-items: center; background: #fff; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
