...
<template>
  <div class="me-page"><div class="me-layout">
    ...
    <main class="me-content">
      <div class="content-header"><h2>📅 我的预约</h2>
      ...
      <el-table :data="list" v-loading="loading" style="width: 100%">
        ...
        <el-table-column label="宠物" width="100"><template #default="{ row }">{{ row.petType === 'Cat' ? '🐈' : '🐕' }} {{ row.petName }}</template></el-table-column>
        ...
        <el-table-column label="操作" width="100">
          <template #default="{ row }"><el-button v-if="row.status === 0 || row.status === 1" size="small" type="danger" @click="cancelOne(row.id)">取消</el-button></template>
        </el-table-column>
      </el-table>
...
const list = ref<any[]>([]); const loading = ref(false); const filterStatus = ref('')
const fetchList = async () => {
  loading.value = true
  try { const params: any = {}; if (filterStatus.value !== '') params.status = filterStatus.value; const res: any = await getMyAppointments(params); list.value = res.data || [] } catch {} finally { loading.value = false }
}
const cancelOne = async (id: number) => { await ElMessageBox.confirm('确定取消这个预约吗？', '提示', { type: 'warning' }); try { await cancelAppointment(id); ElMessage.success('已取消'); fetchList() } catch {} }
