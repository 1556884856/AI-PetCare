    ...
    <div class="me-layout">
      ...
      <main class="me-content">
        <div class="content-header">
          <h2>🐾 我的宠物</h2>
          <el-button type="primary" @click="showDialog = true; resetForm()">+ 添加宠物</el-button>
        </div>
        <div class="pet-grid" v-loading="loading">
          <div class="pet-card" v-for="pet in pets" :key="pet.id">
            ...
            <div class="pet-info">
              <h3>{{ pet.name }}</h3>
              <p>{{ pet.breed }} · {{ pet.age }}个月 · {{ pet.weight }}kg</p>
              <p v-if="pet.notes" class="pet-notes">📝 {{ pet.notes }}</p>
            </div>
            <div class="pet-actions">
              <el-button size="small" @click="editPet(pet)">编辑</el-button>
              <el-button size="small" type="danger" @click="removePet(pet.id)">删除</el-button>
            </div>
          </div>
    ...
    <el-dialog v-model="showDialog" :title="editingId ? '编辑宠物' : '添加宠物'" width="420px">
      <el-form :model="form" label-position="top">
        ...
        <el-form-item label="类型">
          <el-radio-group v-model="form.type"><el-radio value="Dog">🐕 狗狗</el-radio><el-radio value="Cat">🐈 猫咪</el-radio></el-radio-group>
        </el-form-item>
...
const fetchPets = async () => {
  loading.value = true
  try { const res: any = await getMyPets(); pets.value = res.data || [] } catch {} finally { loading.value = false }
}
...
const removePet = async (id: number) => {
  await ElMessageBox.confirm('确定删除这只宠物吗？', '提示', { type: 'warning' })
  try { await deletePet(id); ElMessage.success('删除成功'); fetchPets() } catch {}
}
