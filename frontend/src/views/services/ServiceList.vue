<template>
  <div class="page">
    ...
    <p class="page-desc">选择最适合毛孩子的洗护服务</p>

    <div class="filters">
      <el-radio-group v-model="filterCategory" @change="fetchServices">
        <el-radio-button value="">全部</el-radio-button>
        <el-radio-button value="Bath">🛁 洗浴</el-radio-button>
        <el-radio-button value="Grooming">✂️ 美容</el-radio-button>
        <el-radio-button value="Spa">💆 SPA</el-radio-button>
        <el-radio-button value="Basic">🩺 基础护理</el-radio-button>
      </el-radio-group>
      <el-radio-group v-model="filterPetType" @change="fetchServices" style="margin-left: 16px;">
        <el-radio-button value="">全部宠物</el-radio-button>
    ...
    <div class="service-grid" v-loading="loading">
      <div class="service-card" v-for="s in services" :key="s.id" @click="$router.push(`/services/${s.id}`)">
        ...
        <div class="card-body">
          <div class="card-tag">{{ categoryLabel(s.category) }}</div>
          <h3>{{ s.name }}</h3>
...
const categoryLabel = (c: string) =>
  ({ Bath: '洗浴', Grooming: '美容', Spa: 'SPA', Basic: '基础护理' } as any)[c] || c
...
const fetchServices = async () => {
  ...
  try {
    ...
    if (filterPetType.value) params.petType = filterPetType.value
    const res: any = await getServices(params)
