...
public class CouponService : ICouponService
{
    ...
    public CouponService(PetCareDbContext db, RedisConnection redis, IMessageBus messageBus)
    {
        _db = db;
        _redis = redis;
    ...
    public async Task<CouponDto> CreateCouponAsync(CreateCouponRequest r)
    {
        var coupon = new Coupon
        {
            ...
            MinOrderAmount = r.MinOrderAmount, ValidFrom = r.ValidFrom,
            ValidTo = r.ValidTo, TotalQuantity = r.TotalQuantity
        };
    ...
    {
        var c = await _db.Coupons.FindAsync(id) ?? throw new Exception("�Ż�ȯ������");
    ...
    {
        var coupon = await _db.Coupons.FindAsync(couponId) ?? throw new Exception("�Ż�ȯ������");
    ...
    public async Task<List<CouponDto>> GetAvailableCouponsAsync(int userId)
    {
        ...
        {
            var used = await _db.UserCoupons.CountAsync(uc => uc.CouponId == c.Id);
            var hasClaimed = await _db.UserCoupons.AnyAsync(uc => uc.UserId == userId && uc.CouponId == c.Id);
    ...
    public async Task<List<UserCouponDto>> GetUserCouponsAsync(int userId, string? status)
    {
        ...
        if (status == "unused")
            ...
            query = query.Where(uc => uc.IsUsed);
